 ...
 * lib/site.ts. Prefers an explicit custom domain once one exists; falls
 * back to Vercel's stable production domain, then the current deployment's
 * own URL, then localhost for local dev. Vercel injects
...
// Kept identical to apps/web/src/lib/site.ts (same brand, same copy) — this
// app is the public marketing site, apps/web is the signed-in product.
...
export { SITE_URL, SITE_NAME, SITE_TAGLINE, SITE_DESCRIPTION };
