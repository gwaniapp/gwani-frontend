...
/** Hamburger + slide-in menu (the same sidebar contents), for screens too narrow for the fixed sidebar. */
function MobileNav() {
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent
				...
				className={`top-0 left-0 h-dvh max-h-none w-[min(19rem,88vw)] max-w-none translate-x-0 translate-y-0 content-start gap-0 rounded-none rounded-r-3xl p-0 shadow-2xl sm:max-w-none sm:p-0 data-[state=open]:zoom-in-100 data-[state=open]:fade-in-100 data-[state=open]:slide-in-from-left data-[state=closed]:zoom-out-100 data-[state=closed]:fade-out-100 data-[state=closed]:slide-out-to-left ${ENTER} ${EXIT}`}
			>
