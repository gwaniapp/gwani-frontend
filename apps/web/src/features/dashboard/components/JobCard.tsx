...
function JobCard({ job }: { job: DashboardJob }) {
	...
	return (
		<Link
			...
			className="group grid gap-y-2 rounded-2xl border border-transparent bg-white p-4 shadow-[0_4px_24px_rgb(0_0_0/0.06)] outline-none transition-colors [grid-template-areas:'title_status'_'price_arrow'_'meta_meta'] [grid-template-columns:minmax(0,1fr)_auto] hover:border-primary-400 focus-visible:border-primary-400 focus-visible:ring-2 focus-visible:ring-primary-200 md:items-center md:gap-x-5 md:gap-y-1.5 md:rounded-3xl md:px-6 md:py-5 md:[grid-template-areas:'title_status_arrow'_'price_status_arrow'_'meta_status_arrow'] md:[grid-template-columns:minmax(0,1fr)_auto_auto]"
		>
